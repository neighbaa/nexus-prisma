...
const expand_tilde_1 = tslib_1.__importDefault(require("expand-tilde"));
const Path = tslib_1.__importStar(require("path"));
const generator_helper_1 = require("@prisma/generator-helper");
const generator_1 = require("../generator");
const Settings_1 = require("../generator/Settings");
const loader_1 = require("../generator/Settings/Gentime/loader");
...
(0, generator_helper_1.generatorHandler)({
    onManifest() {
        return {
            defaultOutput: generate_1.OUTPUT_SOURCE_DIR,
    ...
    async onGenerate({ dmmf, otherGenerators, generator, schemaPath }) {
        ...
        if (generator.isCustomOutput) {
            if (!((_a = generator.output) === null || _a === void 0 ? void 0 : _a.value)) {
                throw new Error(`Failed to read the custom output path.`);
            }
            Settings_1.Settings.Gentime.changeSettings({
                output: {
         ...
         */
        Settings_1.Settings.Gentime.settings.change({
         ...
         */
        (0, loader_1.loadUserGentimeSettings)();
        /**
         * If the output path is some explicit relative path then make it absolute relative to the Prisma Schema file directory.
         */
        if (Settings_1.Settings.Gentime.settings.data.output.directory !== 'default' &&
            !Path.isAbsolute(Settings_1.Settings.Gentime.settings.data.output.directory)) {
            Settings_1.Settings.Gentime.settings.change({
                output: {
                    directory: Path.join(Path.dirname(schemaPath), Settings_1.Settings.Gentime.settings.data.output.directory),
                },
            });
        }
        const prismaClientDmmf = prisma_utils_1.PrismaUtils.externalToInternalDmmf(dmmf);
        (0, generator_1.generateRuntimeAndEmit)(prismaClientDmmf, Settings_1.Settings.Gentime.settings);
