...
export const createWhereUniqueInput = (source, model) => {
    ...
    if (uniqueIdentifierFieldsMissingInData.length > 0) {
        // TODO rich errors
        throw new Error(`Cannot create Prisma Client where unique input because the source data (${inspect(source)}) is missing the following unique identifier fields: ${uniqueIdentifierFieldsMissingInData.join(', ')}`);
    }
